...
def solve_slice(num_edges, num_flows, num_paths, sp, capacity):
    with gb.Env(empty=True) as env:
        ...
        with gb.Model(env=env) as model:
            select_paths = model.addVars(range(num_flows), range(num_paths), lb=0, vtype=gb.GRB.BINARY,
                                         name='select_paths')
            select_flow = model.addVars(range(num_flows), vtype=gb.GRB.BINARY, name='select_flow')
            edge_flow = model.addVars(range(num_edges), range(num_flows), lb=0, ub=1000, vtype=gb.GRB.INTEGER,
                                      name='flow')
            ...
            # constraints
            model.addConstrs(select_paths.sum(q, '*') == 1 for q in range(num_flows))
            model.addConstrs(
                flow[e] == gb.quicksum(edge_flow[e, q] * select_flow[q] for q in range(num_flows))
            ...
            model.addConstrs(edge_flow[e, q] == gb.quicksum(select_paths[q, p] * sp[q, p, e] for p in range(num_paths))
                             for q in range(num_flows) for e in range(num_edges))

            model.optimize()

            # get decision results
            res_selct = np.zeros([num_flows, num_paths], dtype=np.int)
            ...
            sel_flow = np.zeros([num_flows], dtype=np.int)
            for q in range(num_flows):
...
def delay_solver(num_edges, num_flows, num_paths, sp, opy, capacity):
    # mask = np.where(sp_numpy>0,1,0)
    with gb.Env(empty=True) as env:
        env.setParam('OutputFlag', 0)
        env.start()
        with gb.Model(env=env) as model:
            ...
            # mask_q = model.addVars(range(num_flows),range(num_edges), vtype=gb.GRB.BINARY,name='flow_mask')
            # delay_qe = model.addVars(range(num_flows),range(num_edges),vtype=gb.GRB.CONTINUOUS)
            # delay_q = model.addVars(range(num_flows),vtype=gb.GRB.CONTINUOUS,name='flow_delay')
            obj = model.addVar(vtype=gb.GRB.CONTINUOUS, name='obj')
            model.update()
            ...
            #                  for q in range(num_flows) for e in range(num_edges))
            # model.addConstrs(delay_qe[q,e] == delay[e] * mask_q[q,e] for e in range(num_edges) for q in range(num_flows))
            ...
            # get decision results
            res_selct = np.zeros([num_flows, num_paths], dtype=np.int64)
...
def gen_label(graph, flow, ocupy, init_bd, num_paths, target, num_flows=1):
    ...
    sp = np.zeros([num_flows, num_paths, num_edges], dtype=np.int32)
    sp_f = np.zeros([num_flows * num_paths, num_edges])
    paths = []
    idx_list = []
    seqs = []
    for q in range(num_flows):
    ...
    if target == 'utility':
        ...
        res_selct, sel_flow = solve_slice(num_edges, num_flows, num_paths, sp, capacity)
    elif target == 'delay':
        res_selct,delay = delay_solver(num_edges, num_flows, num_paths, sp, ocupy, init_bd)
    else:
        res_selct = np.zeros([num_flows, num_paths], dtype=np.int64)
    # convert to label
    label = np.zeros([num_flows, num_paths], dtype=np.int64)
    for q in range(num_flows):
        for p in range(num_paths):
            if res_selct[q, p] == 1:
                label[q, p] = 1
...
def gen_paths(graph, flow, num_paths, num_flows=1):
    num_edges = nx.number_of_edges(graph)
    ...
    for q in range(num_flows):
        ...
        shortest_path.append(k_sp)
    # print('shortest paths (nodes): ',shortest_path)
    sp = np.zeros([num_flows, num_paths, num_edges], dtype=np.int)
    ...
    paths = list(deepflatten(paths))
    seqs = list(deepflatten(seqs))
    sp = sp.reshape([num_flows * num_paths, num_edges])

    return paths, idx_list, seqs, sp, shortest_path


def solver(graph, shortest_path, flow, occupy, capacity, num_paths, num_flows=1, print_info=False):
    num_edges = nx.number_of_edges(graph)
    edges = pd.DataFrame(nx.edges(graph,nbunch=None), columns=['src', 'dst'])
    sp = np.zeros([num_flows, num_paths, num_edges], dtype=np.int16)
    for q in range(num_flows):
        for p in range(num_paths):
            path = shortest_path[q][p]
            pp = []
    ...
    # capacity = np.where(capacity <= 0, 1, capacity)
    t = time.time()
    label,delay = delay_solver(num_edges, num_flows, num_paths, sp, occupy, capacity)
    t_ = time.time()
